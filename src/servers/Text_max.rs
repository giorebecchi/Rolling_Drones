...
impl Server {
    ...
    fn handle_message(&mut self, fragment: Fragment, session: &u64, packet: Packet) {
        println!("Handling fragment {:?}", fragment);
    ...
    fn handle_ack(&mut self, fragment: Ack, session: &u64) {
        println!("Handling ack fragment {:?}", fragment);
    ...
    fn handle_nack(&mut self, fragment: Nack, position: &u64, session: &u64, packet: Packet) {
        println!("Handling nack fragment {:?}", fragment);
        if let Some(boxed) = self.fragment_send.get_mut(&session) {
                ...
                NackType::ErrorInRouting(id) => {
                    ...
                    match root {
                        ...
                        Some(root) => {
                            ...
                            let fr= Fragment::new(*position, total as u64, self.fragment_send.get(&session).unwrap().dati[destination as usize].0);
    ...
    fn handle_flood_response(&mut self, packet: Packet) {
        println!("Handling flood response {:?}", packet);
        ...
        for i in 0..total-1{
            ...
            match root {
                ...
                Some(root) => {
                    ...
                    let data_to_send = data.dati[i].0;
                    let len = data.dati[i].1;
                    let fragment =  Fragment{fragment_index: i as u64, total_n_fragments: total as u64, length: len, data: data_to_send };
...
pub(crate) fn main() -> () {
    ...
    let direct_neighbors = vec![1, 4, 6];
