...
use crate::GUI::login_window::{AppState, NewDroneSpawned, NodeConfig, NodeEntities, NodeType, NodesConfig, SimWindows, SimulationController};
...
struct SimulationCommandsState {
    // Selected node IDs for operations
                                    ...
                                    if let Some(from_node) = nodes.0.iter().find(|n| n.id == from_id) {
                                        for &connected_id in &from_node.connected_node_ids {
                                            if let Some(connected_node) = nodes.0.iter().find(|n| n.id == connected_id) {
                                                let selected = sim_commands.selected_add_target == Some(connected_id.clone());
                                                if ui.selectable_label(
                                                    selected,
                                                    format!("{:?} {}", connected_node.node_type, connected_id)
                                                ).clicked(){
                                                    sim_commands.selected_add_target = Some(connected_id.clone());
                                                }
                                            }
                                        }
                                    }
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button("Remove Connection").clicked() {
                            if let (Some(from_id), Some(to_id)) =
                                ...
                                // Remove sender from 'from_id' to 'to_id'
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button("Set PDR").clicked() {
                            if let Some(id) = sim_commands.selected_pdr_drone {
                                if let Ok(pdr) = sim_commands.pdr_value.parse::<f32>() {
                                    if (0.0..=1.0).contains(&pdr) {
                                        sim.pdr(id, pdr);
                                        sim_commands.selected_pdr_drone = None;
                                        sim_commands.pdr_value.clear();
                                    } else {
                                        ui.label(RichText::new("PDR must be between 0.0 and 1.0").color(Color32::RED));

                                    }
                                } else {
                                    ui.label(RichText::new("Invalid PDR value").color(Color32::RED));
